...
__(상황)__ caretaker는 originator에 대해 어떠한 실행을 한 후 변경에 대한 실행 취소를 하기를 원한다. 
__(수행)__
1. main에서 originator의 상태를 변경한다.
2. caretaker는 originator에게 memento 특정 상태를 저장(saveToMemento)한다.
3. 사용자는 1,2번 과정을 반복하다가 원하는 savepoint로 되돌리기를 원한다.
4. memento 객체를 originator에게 반환(restoreFromMemento)하여 반환한다.
